...
| **Limiti mappatura automatica dei toni** | Controlla il modo in cui i valori di curvatura devono essere scritti nella texture. Se questa opzione è attivata, l’intervallo di valori verrà normalizzato tra 0 e 1 in base ai valori minimo e massimo rilevati durante il processo di cottura al forno. Se è deselezionata, i valori minimo e massimo vengono definiti manualmente.  **Nota:** quando si cuociono UDIM/Porzioni UV, questo parametro deve essere disabilitato per rendere la mappatura tonale uniforme e non specifica per porzione, altrimenti si potrebbero creare giunture tra ogni texture. Per trovare manualmente i giusti valori min/max, esegui prima il bake con questa impostazione attivata, quindi osserva la console/registro per vedere quali valori sono stati generati dal baker. |
