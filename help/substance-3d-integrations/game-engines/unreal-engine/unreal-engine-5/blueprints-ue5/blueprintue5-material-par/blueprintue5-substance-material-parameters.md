...
Per modificare i parametri float, color(float4) e Boolean substance, utilizzare il [nodo Virgola mobile di input](https://helpx.adobe.com/it/substance-3d/unlisted/documentation/integrations/blueprint-node-reference-151584784.html).
...
1. Creare un set di nodi della Virgola mobile di input e impostare la destinazione come variabile dell&#39;istanza di Grafico Substance. Per visualizzare tutti i risultati potrebbe essere necessario deselezionare la casella Sensibile al contesto nella finestra di ricerca.
1. Nel nodo Imposta Virgola mobile di input impostare l&#39;Identificatore come nome del parametro Substance da modificare.\
...
1. Nel nodo della Virgola mobile di input, trascinare una connessione e creare un nodo Crea array. Il nodo Crea matrice avrà un indice pari a 0. L’indice 0 corrisponde al valore float.
1. Crea un nodo di rendering asincrono o sincrono e collega la linea di esecuzione dalla Virgola mobile Imposta input al nodo di rendering. Impostate le istanze da sottoporre a rendering sulla variabile di istanza di Grafico Substance.\
